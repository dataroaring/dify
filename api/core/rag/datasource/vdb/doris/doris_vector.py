...
class DorisConnectionPool:
    ...
    def close_all(self) -> None:
        ...
        # mysql-connector-python pool handles cleanup automatically
        pass
...
class DorisVector(BaseVector):
    ...
    def __del__(self):
        """Destructor to properly close the connection pool."""
        if hasattr(self, "_pool") and self._pool is not None:
            try:
                self._pool.close_all()
            except Exception as e:
                logger.warning("Error closing Doris connection pool %s", e, exc_info=True)
    ...
    def _streamload(self, data: list[dict]) -> None:
        ...
        # StreamLoad endpoint URL
        url = f"http://{self._config.host}:{self._config.streamload_port}/api/{self._config.database}/{self.table_name}/_stream_load"
        ...
        headers = {
            ...
            "max_filter_ratio": "1.0",  # Allow up to 100% filtered rows for debugging
    ...
    def init_vector(self, dataset: Dataset, attributes: list, embeddings: Embeddings) -> DorisVector:
        ...
        return DorisVector(
            ...
            config=DorisConfig(
                ...
                min_connection=dify_config.DORIS_MIN_CONNECTION,
